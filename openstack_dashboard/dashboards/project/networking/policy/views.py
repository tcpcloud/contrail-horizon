...
from contrail_openstack_dashboard.openstack_dashboard.dashboards.project.networking.\
    policy import forms as project_forms
from contrail_openstack_dashboard.openstack_dashboard.dashboards.project.networking.\
    policy import tables as project_tables


class DetailView(tables.DataTableView):
    table_class = project_tables.RulesTable
    template_name = 'project/networking/policy/detail.html'
...
class UpdateView(forms.ModalFormView):
    ...
    template_name = 'project/networking/policy/update.html'
...
class AddRuleView(forms.ModalFormView):
    ...
    template_name = 'project/networking/policy/add_rule.html'
    ...
    def get_form_kwargs(self):
        ...
        for policy in policies:
                network_policies.append(("{0}:{1}:{2}".format(
                                 policy.fq_name[0], policy.fq_name[1],
                                 policy.fq_name[2]),
                                 "{0} ({1})".format(
                                 policy.fq_name[2], policy.fq_name[1])))
...
class CreateView(forms.ModalFormView):
    ...
    template_name = 'project/networking/policy/create.html'
    success_url = reverse_lazy('horizon:project:networking:index')
